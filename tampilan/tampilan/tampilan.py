...
PENGUJIAN_DIR = BASE_DIR / "pengujian"
...
def load_data():
    ...
    df_indonesia = pd.read_csv(DATASET_DIR / "RTA_Dataset_Indonesia.csv", sep=";")
    df_final = pd.read_csv(DATASET_DIR / "Data_final.csv", sep=";")
    return df_asli, df_indonesia, df_final

df_asli, df_indonesia, df_final = load_data()

def preprocessing():
    df_sebelum_null = pd.read_excel(PREPROCESSING_DIR / "Sebelum_Null.xlsx")
    df_sesudah_null = pd.read_excel(PREPROCESSING_DIR / "Sesudah_Null.xlsx")
    df_transformasi = pd.read_csv(PREPROCESSING_DIR / "Transformasi.csv", sep=";")
    return df_sebelum_null, df_sesudah_null, df_transformasi

df_sebelum_null, df_sesudah_null, df_transformasi = preprocessing()

def hasil_pengujian():
    df_kfold_non_smote = pd.read_excel(PENGUJIAN_DIR / "Kfold_non_smote.xlsx")
    df_kfold_smote = pd.read_excel(PENGUJIAN_DIR / "Kfold_smote.xlsx")
    df_k_non_smote = pd.read_excel(PENGUJIAN_DIR / "k_non_smote.xlsx")
    df_k_smote = pd.read_excel(PENGUJIAN_DIR / "k_smote.xlsx")
    df_jarak_non_smote = pd.read_excel(PENGUJIAN_DIR / "metric_non_smote.xlsx")
    df_jarak_smote = pd.read_excel(PENGUJIAN_DIR / "metric_smote.xlsx")
    df_bobot_non_smote = pd.read_excel(PENGUJIAN_DIR / "weight_non_smote.xlsx")
    df_bobot_smote = pd.read_excel(PENGUJIAN_DIR / "weight_smote.xlsx")
...
# ===============================
    ...
    with col2:
        ...
        with tab1:
            st.image(str(IMAGES_DIR / "Sebelum_Penanganan_Outlier.png"), width=630)
        with tab2:
            st.image(str(IMAGES_DIR / "Sesudah_Penanganan_Outlier.png"), width=630)
...
# ===============================
    ...
    with col1:
        ...
        st.image(str(IMAGES_DIR / "Distribusi Accident.png"), width="stretch")

    with col2:
        st.subheader("Distribusi Keseluruhan")
        st.image(str(IMAGES_DIR / "Distribusi Keseluruhan.png"), width="stretch")
    ...
    with col3:
        ...
        st.image(str(IMAGES_DIR / "Distribusi Korelasi.png"), width="stretch")

    with col4:
        st.subheader("Grafik Batang")
        st.image(str(IMAGES_DIR / "Distribusi_Accident_Bar.png"), width="stretch")
...
# ===============================
    ...
    with col1:
        tab1, tab2 = st.tabs(["Kfold Non Smote", "Kfold Smote"])
        with tab1:
...
# ===============================
    ...
    with col1:
        st.image(str(IMAGES_DIR / "Confusion_matrix_dummy.png"), caption="Dummy Classifier", width="stretch")
        st.image(str(IMAGES_DIR / "Confusion_matrix_knn_manual_non_smote.png"), caption="Confusion Matrix KNN Manual (Non SMOTE)", width="stretch")
        st.image(str(IMAGES_DIR / "Confusion_matrix_knn_manual_smote.png"), caption="Confusion Matrix KNN Manual (SMOTE)", width="stretch")

    with col2:
        st.image(str(IMAGES_DIR / "Confusion_matrix_grid_non_smote.png"), caption="Confusion Matrix Grid Search CV (Non SMOTE)", width="stretch")
        st.image(str(IMAGES_DIR / "Confusion_matrix_grid_smote.png"), caption="Confusion Matrix Grid Search CV (SMOTE)", width="stretch")
    ...
    st.image(str(IMAGES_DIR / "perbandingan_hasil_semua_model.png"), width="stretch")
